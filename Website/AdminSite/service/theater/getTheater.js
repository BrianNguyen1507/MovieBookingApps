...
async function getAndDisplayTheater() {
  try {
    ...
    theaterData.result.forEach((theaterData, index) => {
      ...
      nameCell.textContent = theaterData.name;
      row.appendChild(nameCell);

      const addressCell = document.createElement('td');
      addressCell.textContent = theaterData.address;
      row.appendChild(addressCell);

      const actionCell = document.createElement('td');
      actionCell.innerHTML = `
        <button class="btn btn-primary btn-theater-update" data-id="${theaterData.id}">Edit</button>
        <button class="btn btn-danger btn-theater-delete" data-id="${theaterData.id}">Delete</button>
      `;
  //event 
...
deleteButton.addEventListener('click', async () => {
  try {
    ...
    if (confirmation.isConfirmed) {
      const result = await deleteTheaeterById(theaterData.id);
      if (result) {
        tbody.removeChild(row);
        Swal.fire({
          title: 'Deleted!',
          text: `Theater ${theaterData.name} has been deleted.`,
          icon: 'success',
